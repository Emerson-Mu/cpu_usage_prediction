...
# Leer nombre de instancia
def nombre_instancia():
    instancia=""
    with open('/etc/hostname') as file0:
        lineas = file0.readlines()
        for linea in lineas:
            instancia=linea
            break
...
def uso_memoria():
    ...
    #print("-----------------------")
    #print(dicc_memoria)
    dicc_memoria['UsePercent']=(1-float(dicc_memoria['MemAvailable'])/float(dicc_memoria['MemTotal']))*100
    return dicc_memoria

# Creacion del diccionario con las entradas calculadas
def creacion_diccionario(instancia,porcentajes_cpu,uso_memoria,bandwidth):
    ...
    json_recursos = json.dumps(recursos)

    return json_recursos
...
def json_recursos():
    ...
    dicc_recursos=creacion_diccionario(instancia,dicc_porcentajes_cpu,dicc_memoria,dicc_bw)
    #pprint.pprint(dicc_recursos)

    return dicc_recursos
